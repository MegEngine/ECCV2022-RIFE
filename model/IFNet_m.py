...
class IFNet_m(M.Module):
    ...
    def forward(self, x, scale=[4,2,1], timestep=0.5):
        ...
        for i in range(3):
            if flow is not None:
                flow_d, mask_d = stu[i](F.concat((img0, img1, timestep, warped_img0, warped_img1, mask), 1), flow, scale=scale[i])
                flow = flow + flow_d
                mask = mask + mask_d
            else:
                flow, mask = stu[i](F.concat((img0, img1, timestep), 1), None, scale=scale[i])
        ...
        if gt.shape[1] == 3:
            flow_d, mask_d = self.block_tea(F.concat((img0, img1, timestep, warped_img0, warped_img1, mask, gt), 1), flow, scale=1)
